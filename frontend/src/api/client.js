...
// =====================================================================
const BASE = import.meta.env.VITE_API_URL || 'https://ipakapi.elektronbozor.uz'
...
export const api = {
  ...
  admin: {
    ...
    listUsers: () => req('GET', '/api/admin/users'),
    getAiSettings: () => req('GET', '/api/admin/ai-settings'),
    saveAiSettings: (b) => req('PUT', '/api/admin/ai-settings', b),
    clearAiSettings: () => req('DELETE', '/api/admin/ai-settings'),
    testAiSettings: () => req('POST', '/api/admin/ai-settings/test'),
  },

  market: {
    summary: () => req('GET', '/api/market/summary'),
    candles: (productId, limit = 120, source) =>
      req('GET', '/api/market/candles' + qs({ product_id: productId, limit, source })),
  },

  geo: {
    me: () => req('GET', '/api/map/me'),
    setMe: (b) => req('PUT', '/api/map/me', b),
    features: () => req('GET', '/api/map/features'),
    suggestions: (p) => req('GET', '/api/map/suggestions' + qs(p)),
  },
